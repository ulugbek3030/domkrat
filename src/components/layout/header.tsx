          ...
          <div className="hidden flex-1 max-w-xl md:block">
            <form action="/search" method="GET" className="relative">
              <input
                ...
                className="w-full rounded-lg border border-gray-300 bg-gray-50 py-2 pl-4 pr-10 text-sm focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              />
            ...
            {session?.user ? (
              <div className="hidden items-center gap-2 md:flex">
                ...
                {(session.user.role === "ADMIN" || session.user.role === "MANAGER") && (
                  <Link href="/admin" className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">
                    Админ
                  </Link>
                )}
              </div>
              ...
              <div className="hidden items-center gap-2 md:flex">
                ...
                <Link href="/register" className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">
                  Регистрация
                </Link>
              </div>
            )}
        ...
        <div className="pb-3 md:hidden">
          <form action="/search" method="GET" className="relative">
            <input
              ...
              className="w-full rounded-lg border border-gray-300 bg-gray-50 py-2 pl-4 pr-10 text-sm focus:border-blue-500 focus:bg-white focus:outline-none"
            />
      ...
      <div className="border-t border-gray-100 bg-gray-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <nav className="hidden h-10 items-center gap-6 md:flex">
            <Link href="/catalog" className="text-sm font-medium text-gray-700 hover:text-blue-600">
              Каталог
            </Link>
            <Link href="/vehicle" className="text-sm font-medium text-gray-700 hover:text-blue-600">
              Подбор по авто
            </Link>
            <Link href="/categories/filtry" className="text-sm text-gray-600 hover:text-blue-600">
              Фильтры
            </Link>
            <Link href="/categories/tormoznaya-sistema" className="text-sm text-gray-600 hover:text-blue-600">
              Тормозная система
            </Link>
            <Link href="/categories/podveska" className="text-sm text-gray-600 hover:text-blue-600">
              Подвеска
            </Link>
            <Link href="/categories/masla-i-zhidkosti" className="text-sm text-gray-600 hover:text-blue-600">
              Масла и жидкости
            </Link>
          </nav>
        </div>
